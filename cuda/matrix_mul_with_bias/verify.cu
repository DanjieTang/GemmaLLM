...
void zero_init_tensor(std::vector<float>& tensor){
    for (size_t i = 0; i < tensor.size(); i++){
        tensor[i] = 0;
    }
}
...
void compute_reference(float* A, float* B, float* bias, float* C, int M, int K, int N){
...
bool verify(float* A, float* B, float* bias,
            const std::vector<float>& gpu_result, int M, int K, int N){
    std::vector<float> ref(M * N);
    compute_reference(A, B, bias, ref.data(), M, K, N);

    const float tolerance = 1e-3f;
    for (size_t i = 0; i < gpu_result.size(); i++){
        if (fabsf(gpu_result[i] - ref[i]) > tolerance) return false;
    }
    return true;
}
...
int main(){
    ...
    bool ok = verify(h_matrix_a.data(), h_matrix_b.data(), h_bias.data(),
                     h_matrix_c, M, K, N);
