// Benchmark for the model in model.cu: runs the same forward pass
// (linear -> relu -> linear) N times on the tensors saved by model.py
// and reports GPU time per iteration via CUDA events.
//
// Kernels are copied verbatim from model.cu so the measurement matches.
//
// Usage:
//   nvcc -O3 -arch=native -o benchmark benchmark.cu
...
void forward(
    ...
    float* d_layer1_weights,
    float* d_layer1_bias,
    float* d_layer2_weights,
    float* d_layer2_bias,
    float* d_hidden_tensor,
...
int main(int argc, char** argv){
    ...
    for (int i = 0; i < warmup; i++) {
        forward(
            d_input_tensor, d_layer1_weights, d_layer1_bias,
            d_layer2_weights, d_layer2_bias,
            d_hidden_tensor, d_output_tensor,
    ...
    std::cout << "iterations:          " << iterations
              ...
              << "per-iteration GPU:   " << (total_ms / iterations) * 1000.0f << " us\n";

    CUDA_CHECK(cudaEventDestroy(start));
    CUDA_CHECK(cudaEventDestroy(stop));

    cudaFree(d_input_tensor);
    cudaFree(d_layer1_weights);
    cudaFree(d_layer1_bias);
    cudaFree(d_layer2_weights);
    cudaFree(d_layer2_bias);
    cudaFree(d_output_tensor);
    cudaFree(d_hidden_tensor);
}
