...
// CPU reference softmax for one row (slice) of length dim.
void softmaxCPU(const float* input, float* output, int dim){
...
// Compares GPU output against the CPU reference; returns true if all match.
bool checkCorrectness(const std::vector<float>& input,
    ...
    for (int b = 0; b < batch_size; b++){
        softmaxCPU(input.data() + b * dim, cpu_output.data(), dim);

        for (int i = 0; i < dim; i++){
            float gpu_val = gpu_output[b * dim + i];
            float diff = fabsf(gpu_val - cpu_output[i]);
            if (diff > tolerance){
                if (mismatches < 10){
                    printf("Mismatch at batch %d, index %d: gpu=%f cpu=%f diff=%g\n",
                           b, i, gpu_val, cpu_output[i], diff);
                }
                mismatches++;
            }
        }
    ...
    if (mismatches > 0){
        printf("FAILED: %d mismatches found (tolerance %g)\n", mismatches, tolerance);
        return false;
    }

    printf("PASSED: all %d values match the CPU reference (tolerance %g)\n",
           batch_size * dim, tolerance);
    return true;
}
...
int main(){
    ...
    bool correct = checkCorrectness(h_input_tensor, h_output_tensor, batch_size, dim);
